...
在上一段的epoll事件中，epoll_event的epoll_data是一个共用体，因此不拿显示更多信息，存储不了过多信息
但其中有一个void*ptr可以指向另一块空间，因此为了显示更多个信息，使用Channel来对应一个epoll事件，因此建立Channel类来进行管理
typedef union epoll_data {
    void *ptr;  <------------------用它指向Channel对象
    int fd;
    uint32_t u32;
    uint64_t u64;
} epoll_data_t;
